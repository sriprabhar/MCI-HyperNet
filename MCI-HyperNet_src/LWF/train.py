...
from models import DC_CNN, UnetModelWithPyramidDWPAndDC
import torchvision
...
def create_datasets(args, modeltaskA):
    ...
    current_dataset_types = args.current_dataset_type.split(',')
    #print(args.mask_type,mask_types)
    train_data = SliceData(args.train_path,acc_factors,dataset_types,mask_types,'train',modeltaskA, current_dataset_types,args.device)
    dev_data = SliceData(args.validation_path,acc_factors,dataset_types,mask_types,'validation',modeltaskA,current_dataset_types,args.device)
    ...
    display3_data = SliceDisplayDataDev(args.validation_path,'mrbrain_ir','cartesian','8x')
    #return dev_data, train_data
    return dev_data, train_data, display3_data#display1_data, display2_data

def create_data_loaders(args, modeltaskA):
    dev_data, train_data, display3_data = create_datasets(args,modeltaskA)

    #display1 = [display1_data[i] for i in range(0, len(display1_data), len(display1_data) // 16)]
    display3 = [display3_data[i] for i in range(0, len(display3_data), len(display3_data) // 16)]
...
#    )
    ...
    return train_loader, dev_loader, display_loader3 #display_loader1, display_loader2
...
def build_model(args):
    ...
    #model = UnetModelWithPyramidDWPAndDC(args,1,1,32,3,0.0,3).to(args.device) 
    checkpoint_file='<path to model UnetModelWithPyramidDWPAndDC trained on single cascade best_model.pt>'
...
def main(args):
    ...
    train_loader, dev_loader, display3_loader = create_data_loaders(args,modelA)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        visualize(args, epoch, model, display3_loader, writer, 'IR')
...
def create_arg_parser():
    ...
    parser.add_argument('--current_dataset_type',type=str,help='other than datset type')
