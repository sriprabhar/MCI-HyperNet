...
def load_model(checkpoint_file):
    ...
    decouplemodelcheckpoint_file = 'best_model.pt' #currently this is unused
    model = DC_CNN(args, decouplemodelcheckpoint_file).to(args.device)

    #model = UnetModelWithPyramidDWPAndDC(args,1,1,32,3,0.0,3).to(args.device) # (working code )
...
#    latent_list=[]
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            recons = model(input,input_kspace, gamma_val, acc_factor_string, mask_string,dataset_string)
