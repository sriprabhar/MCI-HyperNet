...
from models import DC_CNN, DecoupleModelOnlyWang_etal,UnetModelWithPyramidDWPAndDC
import torchvision
...
def create_datasets(args):
    ...
    display1_data = SliceDisplayDataDev(args.validation_path,'mrbrain_t1320x320','cartesian','4x')
    display2_data = SliceDisplayDataDev(args.validation_path,'kneeMRI320x320','gaussian','5x')
...
def train_epoch(args, epoch, model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        input, input_kspace, target, gamma_val, acc_factor_string, mask_string, dataset_string,mask = data
        ...
        output = model(input,input_kspace, gamma_val, acc_factor_string, mask_string, dataset_string,mask)
...
def build_model(args):
    ...
    #model = UnetModelWithPyramidDWPAndDC(args,1,1,32,3,0.0,320*320).to(args.device) 
    checkpoint_file='/<path to model run with one cascade to initialize the weights in cascade mode>/best_model.pt'
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        visualize(args, epoch, model, display2_loader, writer,'knee')
