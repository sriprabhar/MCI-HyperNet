...
def load_model(checkpoint_file):
    ...
    decouplemodelcheckpoint_file = 'best_model.pt' # currently unused
    model = DC_CNN(args, decouplemodelcheckpoint_file).to(args.device)

    #model = UnetModelWithPyramidDWPAndDC(args,1,1,32,3,0.0,(240*240)).to(args.device) # (working code )
...
def run_unet(args, model, data_loader):
    ...
    with torch.no_grad():
        for (iter,data) in enumerate(tqdm(data_loader)):
            ...
            input, input_kspace,target,fnames,slices,gamma_val,acc_factor_string,mask_string,dataset_string,mask = data
            ...
            recons = model(input,input_kspace, gamma_val, acc_factor_string, mask_string,dataset_string,mask)
